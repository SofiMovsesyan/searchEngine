...
def complete_query(query):
    ...
    terms = sorted(term_regex.findall(query), key=len, reverse=True)
    term_list = '|'.join([re.escape(t) for t in terms])
    return list(map(lambda x: format_completion(x, term_list), query_completions))

def get_path(filename):
    return os.path.join(os.path.dirname(__file__), filename)

print("Building query completion index ...")

stop_words = set(txt_loader(get_path("../searchindex/datasets/stopwords_en.txt"),False))

titles = get_titles(get_path(f'../searchindex/datasets/{dataset_file}'))
